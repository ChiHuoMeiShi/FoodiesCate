...
- (NSUInteger)ancestorCount
{
    if (cachedAncestors_ != nil) {
        return [cachedAncestors_ count];
    }
    
    if (xmlNode_ != NULL) {
        unsigned int count = 0;
        xmlNodePtr currentParent = xmlNode_->parent;
...
- (NSArray *)ancestors
{
    ...
    if (xmlNode_ != NULL) {
        xmlNodePtr currentParent = xmlNode_->parent;
        while (currentParent != NULL) {
            GDataXMLNode * node = [GDataXMLNode nodeBorrowingXMLNode:currentParent];
            if (array == nil) {
                array = [NSMutableArray arrayWithObject:node];
            } else {
                [array addObject:node];
        ...
        cachedAncestors_ = array;
    }
    return array;
}

- (GDataXMLElement *)parent
{
    ...
    if (xmlNode_ != NULL) {
        ...
        if (currentParent != NULL) {
             node = [GDataXMLNode nodeBorrowingXMLNode:currentParent];
        }
    }
    return (GDataXMLElement *)node;
}
...
- (NSString *)kindDescription
{
    switch (self.kind) {
        ...
        case GDataXMLNotationDeclarationKind:
            ...
            break;
        default:
            return @"Don't konwn Kind!";
            break;
    }
}

- (void)printNode:(GDataXMLNode *)node withString:(NSString *)str
{
